...
class GPG:
    ...
    def removeKey(self, mainWindow, state):
        if state:
            result = self.gpg.delete_keys(mainWindow.removeKeyForm.fingerprintLineEdit.text(),
                                          passphrase=mainWindow.removeKeyForm.passphraseLineEdit.text(), secret=True)
            if result.status == 'ok':
                GPG.key = ''
                return result
            else:
        ...
        if permission is not True:
            ...
            if not self.encryptForm.signCB.isChecked():
                with open(os.path.abspath(selectedFile[0]), 'rb') as file:
                    status = self.gpg.encrypt_file(file, recipients=email,
    ...
    def decrypt(self, passphrase):
        ...
        selectedFile = QFileDialog.getOpenFileName(self, "select your decrypted file",
                                                   "/home/" + os.getlogin() + "/Desktop", filter="*.safe")
        try:
            if selectedFile == '':
    ...
    def exportKey(self, status, armor):
        ...
        if not status:
            ...
            selectedPath = QFileDialog.getExistingDirectory(self, "select your file",
            ...
            privateKey = self.gpg.export_keys(self.exportForm.emailLineEdit.text(), True,
                                              passphrase=self.exportForm.passphraseLineEdit.text(), armor=armor)
            if privateKey == '':
